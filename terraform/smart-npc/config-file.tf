...
resource "local_file" "config-gcp" {
    filename = "../genai/api/smart-npc/config.gcp.toml.template"
...
resource "null_resource" "generate_config_toml" {
  provisioner "local-exec" {
    ...
    command = <<EOF
      cat "../genai/api/smart-npc/config.gcp.toml.template" \
        "../genai/api/smart-npc/config.app.toml.template" \
        > "../genai/api/smart-npc/config.toml"
EOF
  }
  depends_on = [
